    ...
    if bot.get('type') == 'rate_anomaly':
        ...
        # when check_detect = True, when the program exhibits out of normal behaviour
            ...
            bot_text = ('For you to take a note : {m} That below expectation period lasted about '
                          '{d} seconds.').format(m=bot.get('message'), t = round(float(bot.get('rate')),2))
    elif bot.get('type')=='high_entropy':
        if bot.get('check_detect'):
            begin_entropy = datetime.strptime(bot.get('timestamp'), '%Y-%m-%d %H:%M:%S.%f')
            bot_text = ('Alert! The entropy has crossed the threshold.' 'Current entropy: {}').format(bot.get('entropy'))
        else:
            end_entropy = datetime.strptime(bot.get('timestamp'),'%Y-%m-%d %H:%M:%S.%f')
            duration_entropy = (end_entropy-begin_entropy).total_seconds()
            duration_est_entropy=int(round(duration_entropy))
            bot_text='Alert! The normal levels of entropy are restored. The entropy phase lasted for{}'.format(duration_est_entropy)
    elif bot.get('type')=='unusualness':
        bot_text=(u'Alert! An unlikely event seems to have taken place, \'{}\',has been observed').format(bot.get('message'))
    else:
        continue
